<x-app-layout>
    <x-slot name="header">
        <div class="relative bg-green-700 text-white text-center py-10 md:py-14 shadow-lg">
        ...
        <div class="max-w-7xl w-full px-4 md:px-8">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6" id="productList">
                @foreach([
                    ...
                    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-lg overflow-hidden p-5 sm:p-6 flex flex-col items-center text-center transition-transform transform hover:scale-105 hover:shadow-2xl product-card border-4 border-green-500">
                        <!-- Imagen del producto -->
                        ...
                        <h4 class="text-lg md:text-xl font-bold text-green-700 mt-3 product-name">{{ __($product[0]) }}</h4>
                        <p class="text-gray-600 dark:text-gray-400 text-sm md:text-base">{{ __($product[2]) }}</p>
                        <p class="text-green-700 font-bold mt-2 text-lg md:text-xl">$ {{ __($product[3]) }} MXN/kg</p>

                        

                        <div class="mt-4 flex space-x-2">
                            <!-- 🛒 Añadir al carrito -->
                            <button wire:click="addToCart('{{ addslashes($product[0]) }}', '{{ $product[3] }}')" 
                               class="px-4 py-2 bg-green-700 text-white text-sm md:text-base font-semibold rounded-full shadow-lg hover:bg-green-800 transition-all duration-600">
            ...
            <div class="text-center mt-8 md:mt-12">
                <a href="{{ route('products.index') }}" class="inline-block px-6 md:px-8 py-3 md:py-4 bg-green-700 text-white text-lg md:text-xl font-bold rounded-lg shadow-lg hover:bg-green-800 transition-all duration-300">
    ...
    <!-- 🔍 Script para Filtrar Productos y Contar Votos -->
    <script>
        ...
        function vote(index) {
            let voteSpan = document.getElementById("votes-" + index);
